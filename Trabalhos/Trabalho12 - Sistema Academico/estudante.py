...
class CtrlEstudante():       
    ...
    def consultaEstudante(self):
        self.limiteIns = LimiteConsultaEstudantes(self)
    ...
    def consultarHandler(self, event):
        ...
        for est in self.listaEstudantes:
            cont += 1
            if  est.nroMatric ==nroMatric:
                msg = est.nroMatric + ' - ' + est.nome
                self.limiteIns.mostraJanela('Dados do Estudante', msg)
                break
            elif cont == len(self.listaEstudantes):
                self.limiteIns.mostraJanela('Erro', 'Estudante não encontrado!')
                break
