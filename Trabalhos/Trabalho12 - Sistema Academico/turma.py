...
class LimiteConsultaTurmas(tk.Toplevel):
    def __init__(self, controle):
        ...
        self.title("Consulta Turma")
        self.controle = controle

        self.frameTurma = tk.Frame(self)
        self.frameButton = tk.Frame(self)
        self.frameTurma.pack()
        self.frameButton.pack()
      
        self.labelTurma = tk.Label(self.frameTurma,text="Código da Disciplina: ")
        self.labelTurma.pack(side="left")  

        self.inputTurma = tk.Entry(self.frameTurma, width=20)
        self.inputTurma.pack(side="left")             
...
class CtrlTurma():       
    ...
    def consultaTurmas(self):
        self.limiteIns = LimiteConsultaTurmas(self)

    def limparHandler(self, event):
        self.limiteIns.inputTurma.delete(0, len(self.limiteIns.inputTurma.get()))

    def fechaHandler(self, event):
        self.limiteIns.destroy()

    def consultarHandler(self, event):
        codDisc = self.limiteIns.inputTurma.get()
        cont1 = 0
        cont2 = 0
        msg = 'Turmas: \n'
        for dis in self.listaTurmas:
            cont1 += 1
            if dis.disciplina.codigo == codDisc:
                msg += dis.codigo + '\n'
                cont2 += 1
            elif cont1 == len(self.listaTurmas) and cont2 == 0:
                self.limiteIns.mostraJanela('Erro', 'A disciplina digitada não possui turmas cadastradas!')
                break
        if cont2 > 0:
            self.limiteIns.mostraJanela('Turmas', msg)
